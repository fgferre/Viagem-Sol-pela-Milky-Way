# Pendências — ARQUIVO (o museu)

> **MUSEU — consulta pontual por número (grep), NUNCA leitura completa por
> agente. O vivo é o [`PENDENCIAS.md`](PENDENCIAS.md).**

Aqui mora a história COMPLETA e VERBATIM dos itens que fecharam, com o
**número intacto** — número é identidade, e nenhum se reaproveita. Nada
aqui exige obra ou decisão do dono; se exigisse, estaria no vivo.

**Como se consulta, e é assim que se consulta:**

```
grep -n '^\*\*61\.' docs/PENDENCIAS-ARQUIVO.md     # acha o item
sed -n '/^\*\*61\./,/^\*\*70\./p' docs/PENDENCIAS-ARQUIVO.md   # lê só ele
```

**Quem lê este arquivo inteiro está gastando ~50 mil tokens para não
aprender nada sobre o que falta fazer.** O vivo responde "o que falta";
este responde "por que isto é assim", uma pergunta de cada vez.

Nasceu em 2026-08-25, pelo **item 98** do vivo — a reforma que o dono
decidiu em 14/08 e mandou executar em 25/08. O que entrou aqui saiu do
`PENDENCIAS.md` sem uma vírgula de diferença; o vivo ficou com uma linha
por item, apontando para cá.

---

## O DIÁRIO DO CABEÇALHO — bastões de 17/08 a 01/09, a fila e os números aposentados

*(Era o topo do `PENDENCIAS.md` até 25/08. A fila que ainda vale mora no
vivo, em forma curta; o que segue é o registro inteiro, incluindo as
seções de números aposentados — que é o que se vem procurar aqui.)*
...
### Bastões de 29/08 a 01/09 (movidos em 02/09)
...
## Item 70 — O ponto que morre seco na borda — a metade que fechou (faixa de guarda, cessão, faixa de instrumento)
...
## Item 77 — As linhas de órbita, ligadas por padrão
...
## Item 82 — Os nomes na tela estão muito intrusivos — os degraus N1 e N2
...
- **N1 — nomes magros, e a régua de relevância ANTES da geometria.**
  ...
  **ELE DECIDIU (24/08): nenhuma das três saídas baratas — o caso é do
  ...
  entrar no motor de filmes". E refinou duas vezes no mesmo dia até a
  forma final: **DIRIGIR, não desligar.** A regra geral continua viva
  por baixo — ela segue governando o que o roteiro não fala (os nomes
  de fundo). O que o roteiro DECLARA assume a frente: o motor (item 75)
  dirige câmera, zoom, lente, legenda e nome, e o beat que nomeia as
  Três Marias garante as três na tela. E o modo pode ser ESCOLHIDO
  MANUALMENTE se alguém quiser — o visitante não perde a soberania
  sobre a própria tela (a gaveta já lhe dá a chave dos nomes; o modo
  dirigido é o padrão do filme, não uma prisão). Não é segundo
  universo: o MUNDO é um só (órbitas, luz, física idênticas nos dois
  modos); quem nomeia é direção, e direção é assistência declarada.
  Até o 75 pousar, o estado fica como está (Alnilam sem nome em t=45),
  aceito como provisório; o caso ALNILAM é o caso de prova do
  vocabulário novo.
...
## Item 83 — Órbitas AAA — a fita e o foco (L1 e L2)
...
### A CONTINUAÇÃO — o modo real, a Q14 e o fecho de 26/08

*(O que está acima veio para o museu em 25/08, quando o item ainda esperava
conferência. O que segue é o texto do vivo, verbatim, na véspera da saída:
o item **FECHOU em 26/08**, na Sala de Conferência — **C15, confirmado como
bom**, a chapa do modo real com a **R1** que ele escolheu na **Q14**.)*
...
## Item 93 — O brilho assistido tinha de ser o algoritmo do NASA Eyes — a receita, a calibração e a C1

*(A receita pousou em 25/08 e ele a APROVOU — "a calibracao nao muda a
ideia central"; a calibração pousou em 26/08, com a **C1** escolhida na
**Q13**. **FECHOU em 26/08** porque nada mais restava dentro dele: a
ressalva que ele deixou junto com a escolha virou o item **104** e voltou
conferida (**C14**). O que segue é o texto do vivo, verbatim, na véspera da
saída.)*
...
**A CONFERÊNCIA DELE CHEGOU NA SEGUNDA LEVA DE 25/08, E AS QUATRO
PERGUNTAS ESTÃO RESPONDIDAS** — ficam escritas como foram feitas, com a
resposta dele colada em cada uma:
...
- **(d)** O véu palha está na dose? Ele é DISCRETO por construção — a
  ...
  **A RESPOSTA DELE (Q12):** *"a dose do Eyes está boa (sutil)"*.

**⚠ A LEITURA DESSAS RESPOSTAS QUASE SAIU ERRADA, E ELE MESMO A
CORRIGIU — no mesmo dia, verbatim:**

> *"nao concordo, acho que isso é só calibração... nao quero dizer que
> nao aprovei a receita do nassa eyes... quero dizer que quero calibrar
> melhor... muitas coisas podem diferir o resultado final... a
> calibracao nao muda a ideia central."*

**Ou seja, e isto não se re-litiga: A RECEITA DO EYES ESTÁ APROVADA, e a
IDEIA CENTRAL FICA.** As quatro peças de cima continuam de pé como
estrutura — Sol pleno em `assistida`, lanterna de leitura somada depois
do Sol, terminador logístico, véu no limbo. Três vereditos seguidos sobre a
DOSE (Q9, Q10, Q11) **não** são reprovação da receita, e ninguém os
transforme em "a casa inventa dose própria" nem em "o freio do 91 volta
porque a receita falhou": isso seria desfazer trabalho que ele aprovou.

**O QUE NASCE, ENTÃO, É CALIBRAÇÃO — mexer nos BOTÕES de dentro da
receita, sem tocar na estrutura dela.** São três, e cada um casa com um
veredito:
...
passa a exigir pedido novo dele.

**E A FOLHA É UMA SÓ, com o MODO REAL junto — o (c) do item 91.** Ele
reafirmou hoje (Q2) o que já dissera: o `?luz=real` está *"escuro demais,
repensar"*. **As calibrações do assistido e as propostas do modo real saem
na MESMA folha de fotos**, porque são a MESMA família de pergunta —
*quanto de honestidade × quanto de leitura*. Julgadas em folhas separadas,
ele escolheria a legibilidade do assistido sem ver o que sobra para o
real. As duas fronteiras do 91 continuam de pé do lado do real: teto de
brilho é **PROIBIDO** em letra pelo `NORTE.md`, e a penumbra FÍSICA no
globo não se revoga — o que se estuda ali é a **exposição do quadro**.
...
`capturas/`** — a razão MUDOU em 25/08: elas já serviram (é por elas que
ele respondeu Q9–Q12), e agora ficam porque são a **referência medida**
contra a qual as calibrações candidatas vão ser comparadas. Apagá-las é
perder o lado "antes" da folha que a fila pede.
...
## Item 103 — A porta do BRILHO no selo parou de virar

*(Aberto e FECHADO em 26/08, no mesmo dia, por obra. O título desta seção
entrou em 26/08, com a chegada do item **104** logo abaixo; o texto é o que
veio do vivo em `39ac684`, sem uma vírgula de diferença.)*
...
## Item 104 — A transição da sombra dos anéis para a noite

*(Nasceu em 26/08 da ressalva que ele deixou junto com a escolha da **C1**
na **Q13**, e a obra pousou no mesmo dia. **FECHOU em 26/08**, na Sala de
Conferência: **C14 — confirmado como bom**. O que segue é o texto do vivo,
verbatim, na véspera da saída.)*
...
**A DECISÃO DELE, VERBATIM, e é ela o contrato desta obra** — chegou pelo
chat em 26/08, quando o P4 esperava o vídeo A/B para escolher entre a
volta na linha do Sol e o turntable no polo:

> *"o que queremos quanto ao movimento de objeto focado é liberdade total
> e responsividade... quero que seja navegação livre e sem travas para
> qualquer dos lados sem nenhum limitador de angulo ou coisa parecida"*

> *"podemos colocar um botao de zerar orientacao, assim como o google maps
> tem um botao de norte"*

**ELA SUPERA O JULGAMENTO QUE ESTAVA MARCADO.** Nem o modelo de então nem
o turntable serviam: os dois são parametrização por DOIS ÂNGULOS, e toda
parametrização por dois ângulos tem polos — em cima do polo um eixo morre
(o horizontal encolhe com `sen φ`) enquanto o outro amplifica, e o
turntable ainda cobrava uma trava de latitude, que é exatamente o
"limitador de angulo" que a frase dele proíbe. O vídeo A/B nunca foi
gravado, e não vai ser: ele escolheu um terceiro caminho que nenhum dos
dois lados filmava.

**O QUE POUSOU** (três commits em 26/08): o acumulador do dedo virou um
QUATERNION guardado no frame da pose de repouso, e o arrasto gira em
torno dos EIXOS DA TELA — horizontal no vertical dela, vertical no
horizontal, os dois compostos num único eixo-ângulo para que a diagonal
não dependa de ordem. Não há grampo em lugar nenhum: o polo se cruza como
qualquer outro ponto, a volta é infinita nos dois eixos e o mundo pode
ficar de cabeça para baixo — **é o desenho, não defeito**. A câmera virou
CORPO RÍGIDO (mira e `up` giram pela mesma rotação), e com isso o roll de
surpresa morreu na origem. A responsividade dos P1–P3 ficou inteira: a
inércia 20/80 com correção de `dt`, a janela de toque e o freio do solo
agora alimentam os incrementos do quaternion.

**A BÚSSOLA**, que é a outra metade da frase dele: botão que só aparece
com o horizonte torto (acende a 5°, apaga a 2° — histerese para não
piscar), endireita em rampa de 0,5 s e **NÃO move a mira**, que é a lei
do botão de norte do Maps. O arrasto cancela a rampa. Mora fora do fluxo,
na borda direita à meia altura — a única região livre nas duas telas —,
então não desconta retângulo útil e a câmera não recua por ela existir.

**O PREÇO, declarado:** giro livre ACUMULA roll (é a holonomia da esfera,
não um bug), e a `volta` deixou de preservar a fase da iluminação — o
trilho que cobrava aquela invariância saiu junto com a lei que a
prometia. É por isso que ele pediu o botão na mesma conversa, e é o botão
que paga esse preço.

**A LIMPEZA PROMETIDA FOI PAGA INTEIRA.** O item prometia que, escolhido
um caminho, o outro saía com a porta; ele escolheu um TERCEIRO, e os dois
saíram. Morreram: a porta `?giro=` (e sua linha no selo e no director),
`lerPortaGiro`, `EixoDoGiro`, `definirEixoDoGiro`, `COLATITUDE_MINIMA_RAD`,
`faixaDaAltura`, `enrolar`, `OrbitaDoVisitante`/`ORBITA_PARADA`,
`direcaoPrivilegiada` e os dois ramos que a porta escolhia. Nada ficou
atrás de flag.

**DE GRAÇA, e estava registrado como obra própria:** o cone de 70°
(`MAX_SOLAR_DEVIATION_GRAUS`) parou de aparar o GESTO no degrau "lua". A
mistura de `direcaoDaLua` virou pose de REPOUSO e o dedo gira ela depois,
então o número voltou a guardar só a mistura calculada — sem mover a pose
de repouso de nenhuma lua.

**AS PROVAS, com carimbo e em disco** (`capturas/item102-giro-livre-ab-*`,
os JSON dos DOIS lados com o carimbo do commit certo — a lição do
auditor: prova sem arquivo não vale, e o tmpdir não é arquivo):

 · **`ab-identidade` BIT-IDÊNTICO, 61 de 61 vistas**, zero DIFERE e zero
   INSTÁVEL, entre `a6b20d0` (a árvore antes desta obra) e `e026b4d`, os
   dois com árvore limpa. Duas vistas (`saturno-anelnb`, `foco-titan`)
   deram INSTÁVEL numa corrida intermediária — é o tremor do anel de
   Saturno que o próprio cabeçalho do `ab-identidade` documenta desde
   25/08; re-medidas em isolamento deram md5 IGUAL **e 0 px de diferença**
   no `diff-pixel`.
 · **`atlas-smoke` exit 0, 171 vereditos**, com a prova 22 nova: a bússola
   nasce apagada e fora do teclado, o arrasto diagonal torce 65,5° e ela
   acende com caixa em (1126, 387) e ZERO sobreposição com o HUD, o clique
   endireita 65,5° → 0,000° e ela apaga sozinha, e a MIRA não anda —
   1,1e-15 de corda unitária. Repetido a 390×844: (330, 400), 0
   sobreposições.
 · **suíte cheia verde** (2.418 testes) e **juiz de a11y verde** (570).
 · A condição de nascimento é cobrada varrendo geometrias DENTRO e FORA
   da faixa da cedência do `up`, mais Urano deitado — a lição que o P4
   pagou, onde a alegação valia para a direção e não para o `up`.

**O QUE SOBRA É SÓ O OLHO DELE, e é no app.** Ele vai testar direto, com o
dedo, **sem vídeo** — a régua desta obra é o tato, e tato não se julga em
gravação. Se o que ele sentir não for o que a frase dele pedia, isso é
item NOVO, com número novo: este fechou.

**A HISTÓRIA COMPLETA — a queixa original, a engenharia reversa do NASA
Eyes e os quatro passos P1–P4 — segue abaixo, verbatim.**
...
## Item 61 — Rever a UI/UX inteira — o fecho (a vista de abertura)

*(Movido verbatim do vivo em 29/08, quando a última ponta fechou. A onda*
*de 22–24/08 tem seção própria mais acima.)*
...
**FECHADO em 29/08, pelo olho e pela obra:** ele escolheu o candidato
**(a) da folha — o sistema inteiro, estilo NASA Eyes** — e a vista
pousou no mesmo dia, junto com a lente do item 86: a abertura nasce no
teto do zoom (~133,7 UA sob 58°), `casaViva` tira raio e direção do
MESMO corpo (o mais externo, no mesmo instante), e na abertura acendem
as cinco órbitas de fora com os nomes Sol, Netuno e Plutão (censo
medido, `atlas-smoke`). Provas da leva no item 86 abaixo.

## Item 86 — A lente da casa: 58° para o app inteiro
...
**FECHADO em 29/08, pelo olho e pela obra, no mesmo dia:** ele cravou
com a foto A/B na mão ("Aprovo — crava o 58°", sabendo do teto que
encurta e da abertura que nasce mais perto). A obra: `ATLAS_FOV_GRAUS`
passou a ser `FOV_DA_CASA` (uma lente só na casa; o pino continua
existindo para o shot pausado não herdar fov), a esfera da abertura
voltou ao sistema inteiro (item 61) e a `BORDA_DO_SISTEMA_INTERNO`
morreu. Números medidos: teto do zoom 226,84 → **133,68 UA** (ui=1;
126,30 em 0,85; 183,73 em 1,4); corpo do Sol a **3,7741 raios** (2,63
mi km, disco de 30,7°; era 6,4042); roda desce em **38 estalos**; na
abertura os cinco de fora acendem as órbitas cheias (0,320) e
Mercúrio apaga, fade monotônico. Provas: A/B de 61 vistas — o filme e
o voo livre **bit-idênticos**, só as 9 vistas de Atlas diferem
(atlas, foco-jupiter/luas/titan/caronte/io, anao-eris-corpo/
anao-vesta-corpo/anao-eris-orbita), titan com tremor herdado já
presente no lado antes; sabotagem por mão independente em worktree —
4 de 4 sabotagens reprovaram exatamente os testes prometidos;
verificação viva no navegador (fov 58, câmera a 132,06 UA na
efeméride de 8/4/2024); suíte cheia 2.461 verdes; `atlas-smoke`
re-pinado com os censos medidos (abertura = Sol, Netuno, Plutão, zero
estrelas; 5,32 décadas; alvo do toque virou Plutão) e a perna do
...
## Item 83 — Órbitas AAA: a fita e o foco — o fecho (B3, a cor)

*(Movido verbatim do vivo em 29/08, quando o item fechou. L1/L2 e o*
*colar do A1 têm seções próprias mais acima.)*
...
**FECHADO em 29/08, pelo olho dele — palavras dele: "ficou lindo,**
**confirmo as fotos — pode fechar o item 83".** O dia do fecho: a
coluna (iv) nasceu das texturas pela receita dele (executor barato +
oráculo 5/5), a prancha v3 de 4 receitas × 2 vistas foi à mesa pelo
instrumento efêmero ?paleta= (worktree, morto após servir), ele
escolheu a (iv), e ela virou código em commit próprio
(world/planetas/corDaTextura.ts + matizDe textura-primeiro; Mercúrio
na fotometria por decisão da própria receita; luas herdam o pai).
Fotos aceitas: item83-b3-cores-v3.png (a prancha) e
item83-b3-pousada-{abertura,jupiter}.png (a cor no app). Re-baseline
das vistas de Atlas declarado; o filme sem linha, bit-idêntico.
OPÇÃO DORMENTE que fica registrada: o G1 (devolver a linha aos oito
sem ponto, HELIO_SEM_PONTO) continua sendo UMA linha, só se ele um
dia quiser — não é pendência.

## Item 89 — O ícone sobrevive ao texto: céu limpo E navegável
...
**FECHADO em 29/08, pelo olho dele — "confirmo as fotos — pode fechar**
**o item 89".** As duas levas do dia: primeiro o buraco original (nomes
desligados = céu inclicável) fechou com a camada `noicones` e o anel
só-ícone (6 alvos clicáveis, zero textos, medido no app vivo); depois,
por ordem dele no mesmo dia, o EYES COMPLETO — anel na cor da órbita
do corpo ao lado do nome, sem risco, camadas independentes de verdade
(nomes, anéis, os dois, ou nada), re-baseline do padrão declarado.
Zero raycast nasceu: o clique segue lendo a lista única do que foi
desenhado (pendência 30), e a armadilha herdada morreu no desenho.
Fotos aceitas: item89-ceu-limpo-navegavel.png e
item89-eyes-completo-ab.png. A pergunta dele sobre labels 3D virou o
item 109.

## Item 97 — A órbita acendia mais cedo no Retina
...
**FECHADO em 29/08, pelo olho dele — "confirmo as fotos — pode fechar**
**o item 97".** Achado em 25/08 pela auditoria da faixa de guarda,
medido e consertado em 29/08: a meia-altura do fade virou px de CSS
(buffer ÷ pixelRatio) e as duas telas passaram a ver o mesmo céu; na
tela comum nada mudou (bit a bit, sentinelas idênticas). A tabela do
dobro das distâncias vive no commit e6c19cf; guarda de invariância
...
## Item 113 — Os minutos dos juízes: cinco fases, −28% na bolsa, o fechamento pesado de 20 a 8,8 min
...
**O FECHO (30/08, mesma noite).** Cinco fases, cada uma provada na hora:
**F1** duplicada morta (`ua40` fica, 15 referências), 4 esperas viram
estado, preços na verdade (catraca 43,3→44,0 — subiu pela honestidade).
**F2/F3** a cirurgia do ab-identidade: carimbo de ÁRVORE (commit sem
mudança de byte não invalida mais nada; o `antes` semeia de qualquer
lado igual — 0 recapturas provadas), captura 1× com re-mira ×2 só na
divergência (pinadas: foco-titan, saturno-anel), Chrome por balde com
navegação limpa, JOBS 3→6 medido, 10 vistas cortadas — lado 7,9→3,5 min,
A/A 50/50 bit-idêntica DUAS vezes (44,0→39,6). **F4** MB1 em 3 baldes
(fronteiraSol+reversao juntas por histerese), 8 famílias, 72 passos:
3,9→1,8 min ×2 corridas, zero acusações (39,6→37,5). **F5** a11y sem
recargas onde o app muda ao vivo (53→13 navegações nas famílias
mexidas, 573=573 vereditos), esperas de estado nos menores, filme com
sentinela das 8 legendas compridas na 2ª largura, cortes (iii), aposta
q=performance MEDIDA e DESCARTADA (0,8 min < piso e 2 vereditos caíram),
vigia da aba intrusa do Chrome (37,5→31,8). **LIMPEZA (Opus)**: a régua
do busca-smoke re-derivada da lei VIVA do enquadramento (lente 58°,
k=3,774, previsto=medido bit-idêntico — a velha reprovava o app por
obedecer ao item 86); a estreia no memoria era a fita da órbita de
MERCÚRIO no primeiro foco interno (43→44, PRESA em 44 por 6 focos:
episódico; a base nasce com ela, com estreia+volta para não carregar
globo); vigia promovido ao `abrirSessao` (targetId; fechar sem reativar
mata a corrida). **PLACAR: bolsa 44,0→31,8 min (−28%); fechamento
pesado ~20→8,8 min; rodadas encadeadas ~5,3** (o `antes` de graça).
Nota honesta: Custo declarado é ordem de grandeza — o memoria mediu
2:08 e 3:37 no MESMO dia conforme a carga; décimo não se caça. **E a
rodada expôs a queda de MODELO** (meio plano semanal de Fable num dia,
"inaceitável" dele): nasceram os operários nomeados de
`.claude/agents/` — `executor` (Opus gravado) e `mecanico` (Sonnet) —
commit `482d777`; a delegação certa virou estrutura, não lembrança.

---

## Item 59 — Trocar de qualidade não trocava a textura dos corpos já carregados

*(FECHADO em 31/08. Commits `47ba249`, `05f1c65`, `e31122a`. O que segue é
o texto do vivo, verbatim, na véspera da saída.)*
...
*(lápide do vivo, movida em 02/09)*
...
**Fecho por prescrição (02/09, ordem dele: "vai na varredura dos 5 ruídos"):** reproduzido UMA vez com o instrumento de hoje — `terralua` 2× em duas sessões, 0 px de 3,08 M — NÃO REPRODUZ. Registro `capturas/varredura-ruidos-02-09.txt`.
...
**Fecho por prescrição (02/09, ordem dele: "vai na varredura dos 5 ruídos"):** reproduzido UMA vez com o instrumento de hoje — `SMOKE=1 JOBS=3` antes e depois, pai sai em <1 s, 0 filho vivo — NÃO REPRODUZ. Registro `capturas/varredura-ruidos-02-09.txt`.
...
**Fecho por prescrição (02/09, ordem dele: "vai na varredura dos 5 ruídos"):** reproduzido UMA vez com o instrumento de hoje — mesma leva, veredito e prompt no mesmo segundo, 0 Chrome órfão — NÃO REPRODUZ. Registro `capturas/varredura-ruidos-02-09.txt`.
...
**Fecho por prescrição (02/09, ordem dele: "vai na varredura dos 5 ruídos"):** reproduzido UMA vez com o instrumento de hoje — `lua` 2× em duas sessões, 0 px — NÃO REPRODUZ. Registro `capturas/varredura-ruidos-02-09.txt`.
...
**Fecho por prescrição (02/09, ordem dele: "vai na varredura dos 5 ruídos"):** reproduzido UMA vez com o instrumento de hoje — `saturno-anel` 4 capturas e `foco-titan` 2, 0 px — NÃO REPRODUZ. Registro `capturas/varredura-ruidos-02-09.txt`.
...
**Fecho (02/09, palavra dele: "achei que isso já estava pronto" — e
estava):** as estações (1), (2) e (3) cumpridas em 31/08 (bloco A:
1.082,9 → 70,1 MiB residentes, `25b131c`/`ee261bb`/`fa44a4a`/`e3324cb`;
bloco B: rampas, oclusores e fita feitos, MSAA reprovado por medida). O
...
**Fecho (02/09, palavra dele):** "foi concluído totalmente" — as três
pernas de 31/08 (relógio do filme, retrato de família, Lua acesa) estavam
prontas e o item guardava 518 linhas de diário por preguiça e teimosia de
quem coordenava, nas palavras dele.
...
**Fecho (02/09, verificado a pedido dele):** o que sobrava — "só a régua
dinâmica" (`b95cfe0`) — chegou com a onda 125 (F2 relevância, F4
encobrimento, F5 tipografia), porque o `Rotulos3d` consome a lista
`alvos` do 2D (`rotulos.ts`: "o 2D segue dono das leis e do anel"). A
beta segue como preferência, pela doutrina de `2cb0296`.
...
**Fecho (02/09, palavra dele):** a reforma existe desde 25/08 (este
próprio arquivo é a prova); o que o item ainda listava (baseline do gate
versionada, dossiê mecânico) é dívida de processo, não de produto, e não
merece número.
...
**Fecho (02/09, verificado a pedido dele):** N3 — "a oclusão em 3D, e ela
fica atrás" — é exatamente o encobrimento da F4 da onda 125 (`09f16e8`:
nome atrás de globo apaga pela rampa de 750 ms, alvo seguido isento); a
...
**Fecho (03/09):** palavra dele depois das pranchas: *"tudo ok, já avaliei"*.
...
**Fecho (03/09):** três fases (Mercúrio/Marte `f4f6db0`; Ceres/Vesta
`a6a6cc4`; cor real de Ceres e Vesta girada `5135978`), Europa/Io zerados
por decisão dele (`804825e`), testes aprovados (`aa9468e`, `a5d35b5`:
suíte 2833 verde). Juiz de identidade 625c86a→5135978: 48 IGUAL; as 6
vistas com Mercúrio/Marte/Vesta/Io DIFEREM por obra (o juiz as marcou
INSTÁVEL com diff 0 — leitura falsa, item **143**; medido pelo
coordenador: atlas-corpo-marte 351/3.249 blocos). Sobra: Fobos/Deimos por
forma real, sem ordem dele. Custo de disco 218 → 320 MB.
...
**Fecho (03/09):** quatro fases (F1 `4823247`, F2 `842d0e9`, F3 `fb2cf14`,
F4 `ff60734`), o padrão virou inglês e o nome do app na capa traduz
(`1d041e5`, `7c07ad8`), palavra dele: "queria que o inglês fosse a língua
padrão", "americano tá bom", "não precisa de fotos para o bilíngue".
Lista do §19 aprovada e rodada (`d4117ba`): 52 testes novos, 18
sabotagens morderam, filme-smoke em inglês 53/53, suíte 2885 verde.
Auditoria de 7 pontos da sessão (925df66..7c07ad8): 1 grave + 9 médios +
5 miúdos, um conserto cada (`8d7cd1d`). Sobras declaradas: ~50 portas de
depuração do selo ficam em pt (só quem digita a porta vê); o `.bin` cru
do perfil do anel fica como piso, como os 11 pares da casa.
