> **Estudo herdado do atlas-orbital** — original: `public/Docs/Tecnologia NASA Eyes_ Câmera e Navegação.md` (doador).
> **Veredito da Onda 0 (2026-08-10): Migra.** Fase consumidora: **Onda 5** (câmera com foco: SLERP + dynamic parenting + damping ≈ espec do lock-on) e **Onda 4** (escala); as técnicas de LOD/instancing servem à **Onda 6**.
> **Procedência:** relatório de "deep research" gerado por IA — 50 notas com data de acesso uniforme. O NASA Eyes é **closed-source**: toda afirmação "o Eyes faz X" aqui é inferência de fontes públicas genéricas (o texto admite "muito provavelmente"). **Não citar como fato sobre o NASA Eyes real**; as fórmulas e padrões em si são corretos e reaproveitáveis.
> **O que vale:** floating origin (V = P − C em dupla precisão na CPU); log-depth com a flag do three.js e o custo declarado (desativa early-Z); SLERP + Bézier cúbica para arco de trajeto seguro; damping ~0,95/quadro; dynamic parenting (M_local = M_pai⁻¹ · M_cam) sem salto visual; InstancedMesh para cinturões; glTF+Draco; LOD por distância. Os point sprites B-V servem só de checagem cruzada — a fotometria da casa já é calibrada por gates.
