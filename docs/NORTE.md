...
|---|---|---|---|
| `ab-identidade.mjs antes` / `depois` | md5 das **60 vistas** oficiais, um lado de cada vez — detector de regressão. Desde 24/08 (item **83**) duas delas têm CORPO EM FOCO (`foco-jupiter` e `foco-luas`): as outras 58 cravam a câmera com `?pos=` ou `?t=`, que é voo livre, e por isso o gate era CEGO a tudo que só muda a cena quando há foco | **~7,9 min por lado** (60 vistas × 2 capturas, `JOBS=3`; derivado da contagem — 60/54 × 7,1 medidos —, re-medir na próxima leva cheia) | fechamento de qualquer mudança que possa mover imagem |
