...
O pilar novo é **o motor estelar**. O NovoSol (14 módulos de `src/three/world/sol/`, 4.960 linhas: convecção GPU, ciclo de Hale/Spörer, flares, CME, coroa volumétrica) deixa de ser um singleton esculpido para o Sol e vira `stellarBody.ts` parametrizado por `{teffK, radiusPc, rotPeriodDays, activityLevel, convective}`. O Sol vira a instância nº 1. **A condição de "gate pixel-igual" que esta linha trazia foi REVOGADA em 2026-08-14** (ver o item 0 de `docs/PENDENCIAS.md`): ela autobloqueava a própria onda, porque trocar o Sol singleton por uma instância do motor genérico move pixel por construção. O gate mede REGRESSÃO, não imobilidade — o veredito passa a ser imagem julgada com o olho, mais rebaseline declarado. **E MEDIDO EM 2026-08-14: esta peça JÁ EXISTE.** `world/stellarBody.ts` (829 linhas) é o adaptador parametrizado, nenhum outro arquivo importa `world/sol/` diretamente, e o Sol da cena já nasce dele (`director.ts:582`, `new StellarBody` com `SOL_PARAMS`). O que falta da Onda 7 não é a peça: são os dois parâmetros declarados sem consumidor em `stellarBody.ts:165-178` — `teffK` e o envelope convectivo —, e ambos exigem editar os 14 vendorizados, o que a regra M3 proibia pela mesma razão de pixel agora revogada. A revisão fechou a peça que faltava: o atlas tem um módulo puro de física estelar (`descriptorFromCatalog`) que roda com **exatamente os dois campos que o `sc1` da viagem já carrega** — `ci` e `logLum`. Temperatura (Ballesteros), classe MK, classe de luminosidade e raio (Stefan-Boltzmann) para as 328.749 estrelas por **zero byte de payload novo**. Este é o caso-modelo da doutrina: TS puro, sem uma única importação de render, julgado por sete estrelas cujo valor está publicado.
...
> orçamento); ~~auto-exposição bidirecional~~ (reprovada em 2026-08-14 — ver
> `LEI-DA-ESTRELA.md` §7; a semente MEDIDA continua válida:
