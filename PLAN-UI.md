...
- **Passos:** (1) maquetes no kit de `capturas/maquetes/` (pele B): M14 mesa 1440×900 com régua + ficha de Saturno, M14b com Busca aberta, M15 celular 390×844 (só o rótulo muda) — portão do dono; (2) implementar: `BarraOuAlcas.tsx` (régua + barra de cima enxuta), CSS (`03-controles.css`, `04-atlas.css`), `App.tsx` (`AREAS_RESERVADAS`), `retanguloDoAtlas.ts` (`direita` da régua), linha de contexto clicável, dicionários (rótulo "Ficha"); (3) `npm run gate`, juízes a11y/busca/atlas (uma rodada cada, regras de economia), `npm run done`, capturas mesa/celular/EN/`shot=2`.
