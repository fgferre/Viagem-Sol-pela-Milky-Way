...
try {
  ...
  //
  // `t=169` é a vista de fora (a `faceon` da leva oficial): é a única em
  ...
  for (const flag of ['nodisc', 'nogdust', 'noglow']) {
    await sessao.ir(`t=169&${PIN}&${flag}=1`);
    const noBoot = await sessao.md5();
    await sessao.ir(`t=169&${PIN}`);
  ...
  await sessao.ir('t=169&ajustes=1&q=cinema&shot=1');
  await sessao.assentar();
