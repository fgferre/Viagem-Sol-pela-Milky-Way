...
const VIEWS = [
  { nome: 'faceon', t: 170, medir: true },
  { nome: 'edgeon', t: 158, medir: false },
];
...
try {
  ...
  const alvo = resolve(OUT, `rodada_${round}_faceon.png`);
  const dom = chrome([
    '--allow-file-access-from-files', '--window-size=900,900',
    '--virtual-time-budget=14000', '--dump-dom',
    `file:///${METRIC.replace(/\\/g, '/')}?a=${alvo.replace(/\\/g, '/')}`,
  ]);

  const bloco = dom.match(/\{\s*"harmonicError"[\s\S]*?\n\}/);
  if (!bloco) throw new Error('métrica não devolveu JSON — o dev server está de pé?');
  const m = JSON.parse(bloco[0].replace(/<[^>]*>/g, ''));
  const faixa = (a) => {
